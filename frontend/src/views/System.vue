...
onMounted(async () => {
  ...
  const jobs = [fetchHealthData(), fetchTokenData(), fetchLogData(), fetchAlerts(), fetchFeedbackData(), fetchToolHealth(), fetchApprovalData(), fetchKnowledgeData(), fetchSensitiveDocuments(), fetchConnectorData(), fetchTaskRuns(), fetchFunnel(), fetchRetention(), fetchNorthStar()]
...
watch(
  ...
  async (value, oldValue) => {
    ...
    if (nextTab === 'retention') {
      await Promise.all([fetchRetention(), fetchNorthStar()])
    }
  }
)
