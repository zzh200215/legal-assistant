...
| 制度 | 要求 | 现状实现 |
|---|---|---|
| 安全负责人 | 明确安全负责人并任命【管理确认】 | 任命文件草稿 docs/security-officer-appointment-draft.md（待签署） |
| 角色分离 | 系统管理 / 审计管理 / 安全管理分离 | 平台角色四级（admin/reviewer/editor/client）+ 资源级鉴权（app/core/auth.py） |
| 职责分工 | 开发/运维/数据岗位职责界定 | 运行手册 docs/operations-runbook.md 覆盖运维职责；任命文件 §2 覆盖安全职责 |
...
| 制度 | 要求 | 现状实现 |
|---|---|---|
| 主动扫描周期 | 每季度一次漏洞扫描【管理确认】 | 上线前必扫（CI 契约门禁 + ruff + pytest 全量，见 §8）；正式部署侧接入云平台漏洞扫描（阿里云云安全中心/基线核查清单 docs/cloud-baseline-hardening-checklist.md） |
| 上线前评估 | 变更发布前安全评估 | CI 四门禁（§8）对每次提交生效 |
| 渗透测试 | 每年一次外部渗透测试【管理确认】 | 正式商用前安排；POC 阶段以等保自评替代 |
| 整改闭环 | 扫描发现项限期修复并复测 | 差距表跟踪（等保自评 §3）；修复后回归测试 |
| 依赖漏洞 | 第三方组件漏洞跟踪 | 依赖锁定（requirements/package-lock）+ 升级评审（§8 供应链安全） |
