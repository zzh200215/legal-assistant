...
from app.services.agent_registry import AGENT_REGISTRY_VERSION, TASK_PROTOCOL_VERSION, get_supervisor_registration, list_agent_registrations
from app.services.agent_harness_service import get_harness_profile
from app.services.agent_skill_registry import SKILL_REGISTRY_VERSION, get_agent_skill, list_agent_skills, resolve_agent_skill
...
def list_runs(
    ...
    if artifact_type and artifact_id:
        matched_runs = agent_service.list_runs_by_artifact(
            db=db,
            user_id=current_user.id,
        ...
        runs = matched_runs[(page - 1) * page_size : page * page_size]
    else:
