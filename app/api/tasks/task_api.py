...
def list_tasks(
    ...
    rows, total = task_service.paginate_visible(
        db=db,
        user_id=current_user.id,
        page=page,
        page_size=page_size,
    ...
    items = [TaskOut.model_validate(task_service.serialize_task(task)).model_dump() for task in rows]
...
def get_task(
    ...
    response.headers["ETag"] = etag_for(task.version)
    return TaskOut.model_validate(task_service.serialize_task(task))
...
def update_task(
    ...
    try:
        ...
        # 乐观锁冲突（版本不匹配 / 并发修改）：透传给全局 handler 映射 409。
        raise
    except Exception as e:
        if should_passthrough_exception(e):
            raise
...
def patch_task(
    ...
    return update_task(task_id=task_id, req=req, if_match=if_match, db=db, current_user=current_user)
...
def get_sub_tasks(
    ...
    if not parent:
        raise api_error(404, "任务不存在", code="TASK_NOT_FOUND")
    sub_tasks, total = task_service.paginate_sub_tasks(
        task_id,
        db=db,
        user_id=current_user.id,
        page=page,
