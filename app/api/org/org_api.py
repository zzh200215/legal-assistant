...
from app.core.versioning import parse_if_match
...
def list_organizations(
    ...
    """查询组织列表（系统管理员可见全部；普通用户仅可见本组织，防止跨租户枚举）"""
    orgs = org_service.list_organizations(db=db)
    if current_user.role != UserRole.admin.value:
        orgs = [org for org in orgs if org.id == current_user.organization_id]
    return orgs
...
def get_organization(
    ...
    """获取组织详情（系统管理员或本组织成员；其他组织按 404 隐藏存在性）"""
    org = org_service.get_organization(db=db, org_id=org_id)
    if not org or (
        current_user.role != UserRole.admin.value
        and org.id != current_user.organization_id
    ):
...
def update_organization(
    ...
    """更新组织（系统管理员）；支持 If-Match 版本冲突校验（409）。"""
...
def list_departments(
    ...
    """查询部门列表（系统管理员可见全部；其他用户仅可见本组织部门，防止跨租户枚举）"""
    if current_user.role != UserRole.admin.value:
        if current_user.organization_id is None:
            return []
        if organization_id is not None and organization_id != current_user.organization_id:
            raise api_error(403, "无权查询其他组织的部门", code="PERMISSION_DENIED")
...
def get_department(
    ...
    """获取部门详情（系统管理员或本组织成员；其他组织按 404 隐藏存在性）"""
    dept = org_service.get_department(db=db, dept_id=dept_id)
    if not dept or (
        current_user.role != UserRole.admin.value
        and dept.organization_id != current_user.organization_id
    ):
