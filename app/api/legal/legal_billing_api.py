...
# ── Serializers（route 层 DTO 映射；隐藏内部/敏感字段）────────────────────────

def _serialize_time_entry(entry: LegalTimeEntry) -> dict:
    return {
        "id": entry.id,
        "organization_id": entry.organization_id,
        "case_id": entry.case_id,
        "operator_id": entry.operator_id,
        "billing_rule_id": entry.billing_rule_id,
...
def _serialize_billing_rule(rule: LegalBillingRule) -> dict:
    return {
        "id": rule.id,
...
def _serialize_invoice(invoice: LegalInvoice) -> dict:
    return {
        "id": invoice.id,
...
def _serialize_payment(payment: LegalPaymentRecord) -> dict:
    return {
        "id": payment.id,
...
def list_time_entries(
    ...
    total = q.count()
    items = q.order_by(LegalTimeEntry.created_at.desc(), LegalTimeEntry.id.desc()).offset((page - 1) * page_size).limit(page_size).all()
    return {"items": [_serialize_time_entry(e) for e in items], "total": total, "page": page, "page_size": page_size}
...
def list_billing_rules(
    ...
    return [_serialize_billing_rule(rule) for rule in q.order_by(LegalBillingRule.created_at.desc()).all()]
...
def list_invoices(
    ...
    total = q.count()
    items = q.order_by(LegalInvoice.created_at.desc(), LegalInvoice.id.desc()).offset((page - 1) * page_size).limit(page_size).all()
    return {"items": [_serialize_invoice(inv) for inv in items], "total": total, "page": page, "page_size": page_size}
...
def list_payments(
    ...
    return [
        ...
        for p in db.query(LegalPaymentRecord).filter(
