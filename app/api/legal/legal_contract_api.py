...
def list_contracts(
    ...
    from app.services.legal.legal_contract_listing import list_visible_contracts

    return list_visible_contracts(
        db,
        org_id=org_id,
...
def expiry_alerts(
    ...
    verify_org_member_access(org_id, current_user.id, db)
    from app.services.legal.legal_contract_listing import list_expiry_alerts

    return list_expiry_alerts(db, org_id=org_id, page=page, page_size=page_size)
...
def contract_diff(
    ...
    from app.services.legal.contract_diff_service import compute_diff
...
def apply_review_suggestions(
    ...
    # 归属校验：审查必须来自本组织的用户（防止跨组织 review_id 非法引用与存在性探测）。
    review_owner = db.query(User).filter(User.id == review.user_id).first()
    if review_owner is None or review_owner.organization_id != contract.organization_id:
        raise HTTPException(404, detail="关联的合同审查不存在")
    if review.case_id:
        try:
            verify_case_access(review.case_id, current_user.id, db)
        except Exception:
            raise HTTPException(404, detail="关联的合同审查不存在")
...
def send_sign_request(
    ...
    try:
        from app.services.legal.signing_provider_service import signing_provider_service
...
def _verify_signing_webhook(provider: str, payload: bytes, signature: Optional[str]) -> None:
    """统一验签（P1-C）：基于原始请求体 HMAC-SHA256 + 常量时间比较。

    协议方（fadada/esigncn）不随回调发送独立时间戳，防重放由事件唯一 ID
    （provider_event_id 唯一约束）承担；payload 内 occurred_at 的异常时序
    检测沿用 sign_callback 中的既有逻辑。
    """
    try:
        secrets_by_provider = json.loads(get_settings().SIGNING_WEBHOOK_SECRETS_JSON or "{}")
    except ValueError:
        secrets_by_provider = {}
    secret = secrets_by_provider.get(provider) or ""
    verifier = WebhookVerifier(str(secret), scheme="raw", encoding="hex")
...
async def sign_callback(
    ...
    try:
        ...
        write_webhook_audit(
            db=db,
            result="blocked",
            provider=f"signing:{provider}",
            reason_code=exc.code,
            sanitized_metadata=json.dumps({"provider": provider}, ensure_ascii=False),
        )
