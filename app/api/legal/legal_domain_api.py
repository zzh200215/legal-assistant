...
from app.services.legal.legal_workspace_service import serialize_workspace_row
...
def _load_owned_row(db: Session, user: User, model, item_id: int, kind: str):
    ...
    - 记录属主：直接可访问；
    - 系统 admin：全局可访问（系统管理页用途）；
    - dept_admin：仅限与本部门管理员同组织的记录（无 case 的记录也必须校验组织，
      防止仅凭系统角色跨租户访问）。
    """
    ...
    if row.user_id != user.id and user.role == "dept_admin":
        ...
        if (
            ...
            or owner is None
            or owner.organization_id != user.organization_id
        ):
            raise api_error(403, "无权访问该记录", code="LEGAL_ACCESS_FORBIDDEN")
    if row.case_id:
        try:
