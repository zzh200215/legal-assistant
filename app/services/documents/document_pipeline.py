...
def _complete_stage(db: Session, key: str, snapshot: Any) -> None:
    try:
        ...
        # 不主动 rollback 外层事务：complete 内部自行 commit；外层可能还有
        # 未提交的阶段状态，整段回滚会破坏幂等键与阶段状态的一致性。
        logging.getLogger(__name__).warning("幂等阶段完成登记失败 key=%s", key, exc_info=True)


def _fail_stage(db: Session, key: str) -> None:
    try:
        idempotency_service.fail(db, scope=_IDEMPOTENCY_SCOPE, key=key)
    except Exception:  # noqa: BLE001 - 幂等失败登记不阻断业务
        logging.getLogger(__name__).warning("幂等阶段失败登记失败 key=%s", key, exc_info=True)
