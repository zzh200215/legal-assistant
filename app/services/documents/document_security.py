...
_EXTENSION_MIME: dict[str, str] = {
    ...
    "bmp": "image/bmp",
    "webp": "image/webp",
    # 旧版 Office 二进制（OLE2 复合文档）：detect_mime 需走 OLE2 魔数校验
...
# OLE2 复合文档（.doc/.xls/.ppt 旧版 Office）：只有魔数可验证，无更细的格式探测。
_LEGACY_OLE2_EXTS = {"doc", "xls"}
...
def detect_mime(head: bytes, filename: str, *, allowed: set[str] | None = None) -> tuple[str, str]:
    """真实 MIME 检测：扩展名白名单 + magic-byte 交叉校验，返回 (ext, mime)。

    ``allowed`` 传自定义扩展名白名单（如法源导入/合同审查端点）；缺省用配置。
    """
    ...
    if ext in _MAGIC_BINARY_EXTS:
        ...
        return ext, _EXTENSION_MIME[ext]
    # 旧版 Office 二进制（.doc/.xls）：仅 OLE2 魔数可验证，不信任扩展名。
    if ext in _LEGACY_OLE2_EXTS:
        if not head.startswith(_OLE2_MAGIC):
            raise DocumentSecurityError(
                "DOCUMENT_MIME_MISMATCH", f"文件头与 .{ext} 不一致（非 OLE2 复合文档）"
            )
        return ext, _EXTENSION_MIME[ext]
    # 文本类型：不得命中二进制签名
...
def spool_upload_to_temp(source: BinaryIO, *, max_bytes: int, suffix: str = ".tmp") -> tuple[Path, int, str]:
    """流式把上传流落到本地临时文件，边读边算 SHA-256，并强制大小上限。

    ``suffix`` 建议传检测出的真实扩展名（如 ``.xlsx``）：部分解析库（openpyxl
    3.1.5 的 load_workbook）按扩展名判定格式，未知后缀会被拒绝。

    返回 (temp_path, size, content_hash)。超限抛 DocumentSecurityError。
    """
    fd, tmp_path = tempfile.mkstemp(prefix="aibg-upload-", suffix=suffix)
...
def secure_spool_file(
    ...
    *,
    ...
    """统一上传校验入口：嗅探 MIME → 流式落盘 → zip-bomb → 病毒扫描。

    供文档上传与各业务上传端点共用，保证安全策略一致：
    1. 内容嗅探 MIME + 扩展名白名单交叉校验（不信任客户端 Content-Type）；
    2. 流式分块落盘临时文件，强制大小上限，禁止整体 read()；
    3. docx/xlsx 只读中央目录 zip-bomb 审查（不实际解压）；
    4. 病毒扫描（未配置扫描器不伪造通过；显式启用但不可用 fail-closed）。

    返回 (temp_path, ext, mime, size, sha256)。成功时调用方负责 finally 清理
    temp_path；校验失败时本函数已清理临时文件并抛 DocumentSecurityError。
    """
    filename = getattr(file, "filename", None) or "upload"
    head = file.file.read(_HEAD_BYTES)
    file.file.seek(0)
    ext, mime = detect_mime(head, filename, allowed=allowed_exts)

    # 临时文件带真实扩展名：openpyxl 等解析库按扩展名判定格式，.tmp 会被拒绝。
    temp_path, size, content_hash = spool_upload_to_temp(
        file.file, max_bytes=max_bytes, suffix=f".{ext}"
    )
    try:
        if ext in _ZIP_BASED_EXTS:
        ...
        return temp_path, ext, mime, size, content_hash
    except BaseException:
        try:
            os.unlink(temp_path)
        except OSError:
