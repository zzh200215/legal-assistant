...
from app.core.llm_governance import LLMGovernanceError
...
class LLMGovernanceService:
    ...
    def get_user_status(self, db: Session, user_id: int) -> dict[str, Any]:
    ...
    def _enforce(
        ...
        *,
        ...
        db = SessionLocal()
        try:
            status = self.get_user_status(db, user_id)
        finally:
            db.close()

        today = status["today"]
        if policy["daily_request_limit"] and today["used_requests"] >= policy["daily_request_limit"]:
            detail = {
                "action": action,
        ...
        if policy["daily_token_limit"] and today["used_tokens"] + estimated_total_tokens > policy["daily_token_limit"]:
            detail = {
                "action": action,
                "limit": policy["daily_token_limit"],
                "used": today["used_tokens"],
                "estimated_request_tokens": estimated_total_tokens,
                "remaining": max(policy["daily_token_limit"] - today["used_tokens"], 0),
            }
        ...
        if policy["rate_limit_window_seconds"] and policy["rate_limit_max_requests"]:
            current_requests = self._increment_rate_counter(
                user_id=user_id,
                window_seconds=policy["rate_limit_window_seconds"],
            )
            if current_requests > policy["rate_limit_max_requests"]:
                detail = {
                    "action": action,
                    "window_seconds": policy["rate_limit_window_seconds"],
                    "limit": policy["rate_limit_max_requests"],
    ...
    def _rate_key(self, *, user_id: int, window_seconds: int) -> str:
        bucket = int(time.time() // max(window_seconds, 1))
        return f"{settings.LLM_LIMIT_REDIS_PREFIX}:rate:{user_id}:{window_seconds}:{bucket}"

    def _increment_rate_counter(self, *, user_id: int, window_seconds: int) -> int:
        if window_seconds <= 0:
            return 0
        key = self._rate_key(user_id=user_id, window_seconds=window_seconds)
        client = self._redis()
    ...
    def _get_rate_counter_value(self, *, user_id: int, window_seconds: int) -> int:
        if window_seconds <= 0:
            return 0
        key = self._rate_key(user_id=user_id, window_seconds=window_seconds)
        client = self._redis()
