...
# 统一验签器错误码 → 支付 Webhook 既有错误码（保持对外契约不变）
_STRIPE_VERIFY_ERROR_MAP = {
    "NOT_CONFIGURED": "WEBHOOK_SIGNATURE_NOT_CONFIGURED",
    "MISSING_FIELD": "INVALID_WEBHOOK_SIGNATURE",
...
class PaymentEventService:
    ...
    def verify_signature(self, raw_body: bytes, signature: str | None,
        ...
        """统一验签（P1-C）：经 WebhookVerifier（Stripe 格式 t=<ts>,v1=<hmac>）。

        常量时间比较 + 时间戳窗口 + fail-closed；对外错误码保持不变。
        """
        ...
        verifier = WebhookVerifier(
            secret,
            scheme="stripe",
            tolerance_seconds=settings.PAYMENT_WEBHOOK_SIGNATURE_TOLERANCE_SECONDS,
        )
        try:
            verifier.verify(raw_body, signature)
        except WebhookVerificationError as exc:
            code = _STRIPE_VERIFY_ERROR_MAP.get(exc.code, "INVALID_WEBHOOK_SIGNATURE")
            raise WebhookRejectedError(code, str(exc)) from exc
    ...
    def handle_webhook(self, *, db: Session, provider: str, event_type: str,
        ...
        """API 入口：验签（失败写安全审计）→ 幂等登记事件（快速返回，异步处理）。"""
        try:
            ...
            write_webhook_audit(
                db=db,
                result="blocked",
                provider=f"payment:{provider}",
                reason_code=exc.code,
                sanitized_metadata=json.dumps({"event_type": event_type or ""}, ensure_ascii=False),
            )
            raise
        data = payload.get("data", {}) if isinstance(payload.get("data"), dict) else {}
        obj = data.get("object", {}) if isinstance(data.get("object"), dict) else {}
