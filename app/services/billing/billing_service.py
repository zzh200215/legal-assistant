...
from app.services.billing.billing_state_machines import invoice_transition, refund_transition
...
class BillingService:
    ...
    def create_invoice(self, *, db: Session, organization_id: int, case_id: int,
                       ...
                       _retry_depth: int = 0) -> LegalInvoice:
        """创建账单：从已确认计时条目自动生成明细行，计算小计/税额/合计。

        并发创建同组织同日账单可能撞 invoice_no 唯一约束：冲突时回滚并
        重算编号重试（_retry_depth 为内部参数，限次防止死循环）。
        """
        if idempotency_key:
        ...
        # 生成账单编号——"今日"以数据库时间为准（与 created_at 同源，避免本地/UTC
        # 日界错位导致同 org 两张发票拿到相同 seq → 唯一约束冲突。契约测试暴露）。
        today_db = db.scalar(sa_func.date(sa_func.now()))
        if isinstance(today_db, str):
            today_db_date = date.fromisoformat(today_db)
        else:
            today_db_date = today_db
        today_str = today_db_date.strftime("%Y%m%d")
        seq = db.query(sa_func.count(LegalInvoice.id)).filter(
            LegalInvoice.organization_id == organization_id,
            sa_func.date(LegalInvoice.created_at) == today_db,
        ).scalar() or 0
        ...
        try:
            ...
            # 并发创建同组织同日账单 → invoice_no 撞唯一约束（或并发相同幂等键）：
            # 回滚后重算编号重试；重试中条目归属校验会自然生效。
            db.rollback()
            if _retry_depth < 3:
                return self.create_invoice(
                    db=db, organization_id=organization_id, case_id=case_id,
                    created_by=created_by, client_display_name=client_display_name,
                    issue_date=issue_date, due_date=due_date,
                    billing_period_start=billing_period_start,
                    billing_period_end=billing_period_end,
                    discount_amount=discount_amount, tax_rate=tax_rate,
                    currency=currency, time_entry_ids=time_entry_ids,
                    idempotency_key=idempotency_key, _retry_depth=_retry_depth + 1,
                )
            raise
        db.refresh(invoice)
    ...
    def record_payment(self, *, db: Session, invoice_id: int, organization_id: int,
        ...
        """记录一笔付款，幂等处理。"""
        # 行级锁串行化同一账单的并发入账（支付网关重试/乱序重复投递）：
        # transaction_id 去重与超额校验在同一临界区内生效，防止重复入账。
        invoice = (
            db.query(LegalInvoice)
            .filter(LegalInvoice.id == invoice_id)
            .with_for_update()
            .first()
        )
        if not invoice:
    ...
    def approve_refund(self, *, db: Session, refund_id: int, approved_by: int,
        ...
        # 集中校验 + 数据库原子保证：amount <= 已付已确认 - 已退(pending+completed，排除本条)
        # 使用派生表包装以兼容 MySQL（禁止在 UPDATE 子查询中直接读取被更新的表，报错 1093），
        # 同时把付款与退款分开求和，避免 LEFT JOIN 多付款行导致的重复计数。
        ...
        result = db.execute(sa_text(
            ...
            "  SELECT avail FROM ("
            "    SELECT COALESCE((SELECT SUM(amount) FROM legal_payment_records "
            "                     WHERE invoice_id = :inv AND status='confirmed'), 0)"
            "         - COALESCE((SELECT SUM(amount) FROM legal_refund_records "
            "                     WHERE invoice_id = :inv AND status IN ('pending','completed') "
            "                     AND id <> :rid), 0) AS avail"
            "  ) AS _avail_check"
            ")"
