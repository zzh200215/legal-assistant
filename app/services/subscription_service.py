...
class SubscriptionService:
    ...
    def release_usage(self, *, db: Session, usage_event_id: str) -> UsageReservation | None:
        ...
        reservation_transition(reservation.status, "released")
        reservation.status = "released"
        db.commit()
        return reservation
    ...
    def expire_stale_reservations(self, *, db: Session, limit: int = 200) -> int:
        ...
        for reservation in stale:
            self.release_usage(db=db, usage_event_id=reservation.usage_event_id)
            reservation = db.query(UsageReservation).filter(
                UsageReservation.usage_event_id == reservation.usage_event_id).first()
            if reservation:
                reservation_transition(reservation.status, "expired")
