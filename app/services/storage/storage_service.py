...
from app.services.storage.storage_base import StorageAdapter, StorageBackendUnavailable


def _harden_path(path: Path, mode: int) -> None:
    """POSIX 下收紧权限：文件 0600 / 目录 0700（不可执行、最小权限）。

    Windows 无 POSIX 权限位，no-op；目录/文件 ACL 属部署方职责（文档说明）。
    """
    if os.name != "posix":
        return
    try:
        ...
        # 权限收紧失败不阻断业务（mkstemp 已保证 0600），由部署方巡检。
        pass
...
class LocalStorageAdapter:
    ...
    def put_stream(self, key: str, source: BinaryIO, *, content_type: str | None = None) -> dict:
        ...
        _harden_path(target.parent, 0o700)
