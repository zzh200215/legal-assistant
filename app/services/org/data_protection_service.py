...
class DataProtectionService:
    ...
    RULES = (
        ...
        # 中文地址（保守形态：行政区划 + 路/街/大道 + 数字门牌，可选 栋/单元/室）。
        # 只匹配"路/街/大道 + 号"的完整地址形态，避免把公司名称等普通文本误判为地址。
        SensitiveRule(
            "cn_address",
            "地址",
            "medium",
            re.compile(
                r"(?:[\u4e00-\u9fa5]{2,8}(?:省|自治区|市|自治州))?"
                r"(?:[\u4e00-\u9fa5]{2,8}(?:市|区|县|自治县|旗))?"
                r"(?:[\u4e00-\u9fa5]{2,10}(?:路|街|大道|巷|道))"
                r"[0-9０-９]{1,6}号"
                r"(?:[0-9０-９A-Za-z]{0,3}(?:栋|幢|号楼|单元))?"
                r"(?:[0-9０-９A-Za-z]{0,3}室)?"
            ),
            lambda value: _mask_middle(value, keep_start=2, keep_end=2),
        ),
