...
class RerankerTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_build_heuristic_by_default(self):
        ...
        from app.services.rag.rerank import HeuristicReranker, build_reranker
        with patch("app.services.rag.rerank.settings.RAG_RERANK_ENGINE", "heuristic"), patch(
            "app.services.rag.rerank.settings.RAG_LLM_RERANK_ENABLED", False,
        ):
    ...
    def test_build_bge_when_engine_bge(self):
        from app.services.rag.rerank import BGEReranker, build_reranker
        with patch("app.services.rag.rerank.settings.RAG_RERANK_ENGINE", "bge"), patch(
            "app.services.rag.rerank.settings.RAG_LLM_RERANK_ENABLED", False,
        ):
            reranker = build_reranker(rag_service)
    ...
    def test_build_llm_when_legacy_flag_enabled(self):
        from app.services.rag.rerank import LLMReranker, build_reranker
        with patch("app.services.rag.rerank.settings.RAG_LLM_RERANK_ENABLED", True):
            reranker = build_reranker(rag_service)
        self.assertIsInstance(reranker, LLMReranker)

    async def test_heuristic_delegates_to_rerank_candidates(self):
        from app.services.rag.rerank import HeuristicReranker
    ...
    async def test_llm_rerank_reorders_and_attaches_scores(self):
        ...
        from app.services.rag.rerank import LLMReranker
    ...
    async def test_bge_rerank_reorders_with_scores(self):
        ...
        from app.services.rag.rerank import BGEReranker
